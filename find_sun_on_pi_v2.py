...
def main(argv):
	...
	bright_values = {}
	count = 0
	# Initially this value was set to 73
	step_scan = 86
	position_scan = 0

	# Currently the scanning is implemented by having the camera turn 360 degrees. 
	# TODO: Have to implement the same thing while preventing the camera cable from being twisted
	while True:
        	if count == 6:
               		break
        	frame = vs.read()
        	...
        	time.sleep(1)
        	bright_values[count] = out

        	count = count + 1

        	t=Stepper(0,0)
        	t.backwards(int(Stepper.delay) / 1000.0, int(step_scan))
		position_scan = position_scan + step_scan
